...
export default function ResetPassword() {
  ...
  const onOtpSubmit = async (data: OtpFormData) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:3000/verify-password-reset",
        {
          method: "POST",
  ...
  const handleOtpResend = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:3000/request-password-change",
        {
          method: "POST",
  ...
  // If token is missing, show an error shell
  if (!token) {
    return (
      <AuthShell
      ...
      >
        <div className="text-center py-6 space-y-4">
          ...
          <p className="text-sm text-ink-500 dark:text-ink-400">
            This reset link is missing a security token. Please request a new
            link to proceed.
          </p>
          <Link
            to="/forgot-password"
